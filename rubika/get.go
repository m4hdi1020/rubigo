...
func (b bot) GetMessageAllWebsocket(handler func(*WebSocketResponse)) {
	...
	if err != nil {
		fmt.Printf("ERROR Encrypting Data !---!\nERROR MESSAGE: %s\n", err.Error())
		return
	}
	send := send{
		ApiVersion: "5",
	...
	conn, _, err := websocket.DefaultDialer.Dial("wss://jsocket1.iranlms.ir:80/", nil)
	if err != nil {
	...
	if err != nil {
		fmt.Printf("ERROR Reading Data From Websocket !---!\nERROR MESSAGE: %s\n", err.Error())
		return
	}
	err = conn.WriteJSON(send)
	if err != nil {
		fmt.Printf("ERROR Sending Data To Websocket !---!\nERROR MESSAGE: %s\n", err.Error())
		return
	}
	for {
		var jsonResp map[string]string
		err = conn.ReadJSON(&jsonResp)
		if err != nil {
			fmt.Printf("ERROR Reading Data From Websocket !---!\nERROR MESSAGE: %s\n", err.Error())
			return
		}
		decodeJson, err := encryption.Decrypt(jsonResp["data_enc"])
		if err != nil {
			fmt.Printf("ERROR Decrypting Data !---!\nERROR MESSAGE: %s\n", err.Error())
			return
		}
		decodeStr := string(decodeJson)
		if strings.Contains(decodeStr, "\"chat_updates\"") {
			var resp webSocketResponse
			err = json.Unmarshal(decodeJson, &resp)
			if err != nil {
				fmt.Printf("ERROR Unmarshaling Data !---!\nERROR MESSAGE: %s\n", err.Error())
				return
			}
			handler(&resp.Chats[0])
		}
	}
}
