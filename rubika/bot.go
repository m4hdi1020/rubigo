...
var (
	...
	rubikaAPIList = []string{"https://shadmessenger148.iranlms.ir/"}
)
...
type sendMessageReply struct {
	...
	Input  struct {
		...
		Rnd            string `json:"rnd"`
		Text           string `json:"text"`
		ReplyToMessage string `json:"reply_to_message_id"`
	} `json:"input"`
	Clinet struct {
		AppName    string `json:"app_name"`
		AppVersion string `json:"app_version"`
...
func newSendFile(text, guid, dcId, fileId, fileName string, size int, accessHashRec string, replyTOMessageId string) (string, error) {
	data := sendFile{
		Method: sendMessage,
		Input: struct {
			ObjectGUID string "json:\"object_guid\""
			...
			Mime          string "json:\"mime\""
			AccessHashRec string "json:\"access_hash_rec\""
		}{DcID: dcId, FileID: fileId, Type: "File", FileName: fileName, Size: size, Mime: "mime", AccessHashRec: accessHashRec}, Text: text, ReplyToMessage: replyTOMessageId},
		Client: struct {
			AppName    string "json:\"app_name\""
			AppVersion string "json:\"app_version\""
			Platform   string "json:\"platform\""
...
func newSendImage(guid string, caption string, dcId string, id string, fileName string, size int, width int, height int, accessHashReq string, replyToMessage string) (string, error) {
	file, err := os.Open("/home/echo/Pictures/Thumb.txt")
	if err != nil {
		log.Fatalln(err)
	}
	defer file.Close()
	th, _ := io.ReadAll(file)
	data := imageData{
		Method: sendMessage,
		Input: struct {
			ObjectGUID string "json:\"object_guid\""
			...
			Height        int    "json:\"height\""
			AccessHashRec string "json:\"access_hash_rec\""
		}{DcID: dcId, FileID: id, Type: "Image", FileName: fileName, Size: size, Mime: "mime", ThumbInline: string(th), Width: width, Height: height, AccessHashRec: accessHashReq}, Text: caption, ReplyToMessage: replyToMessage},
		Client: struct {
			AppName    string "json:\"app_name\""
			AppVersion string "json:\"app_version\""
			Platform   string "json:\"platform\""
...
func newSend(auth string, dataEnc string) (map[string]string, error) {
	...
	for i := 0; i < 4; i++ {
		resp, err := http.Post(rubikaAPIList[0], jsonContentType, bytes.NewBuffer(dataJson))
		if err != nil {
			if i == 3 {
				return nil, err
			}
...
func newSendMessage(text string, guid string) (string, error) {
	data := sendMessageStruct{
		Method: sendMessage,
		Input: struct {
			ObjectGuid string "json:\"object_guid\""
			Rnd        string "json:\"rnd\""
...
func newSendMessageReply(text string, guid string, messageId string) (string, error) {
	data := sendMessageReply{
		Method: sendMessage,
		Input: struct {
			ObjectGuid     string "json:\"object_guid\""
			Rnd            string "json:\"rnd\""
