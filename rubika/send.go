...
func (b bot) SendFile(guid string, fileName string, data io.Reader, caption string , messageID string) error {
	...
	if size <= 131072 {
		...
		dataEnc, err := newSendFile(caption, guid, dcId, id, fileName, size, hashReq.Data.AccessHashRec , messageID)
		if err != nil {
			return err
		}
		body, err := newSend(b.Auth, dataEnc)
