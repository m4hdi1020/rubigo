...
func (b bot) SendFile(guid string, fileName string, data io.Reader, typeFile string, caption string) error {
	...
	id, dcId, hashAccess, url, err := getInfoSendFile(fileName, size, typeFile, b.Auth)
	if err != nil {
		return err
	}
	if size <= 131072 {
		...
		dataEnc, err := newSendFile(caption, guid, dcId, id, fileName, size, typeFile, hashReq.Data.AccessHashRec)
		if err != nil {
			return err
		}
		body, err := newSend(b.Auth, dataEnc)
...
func (b bot) SendImage(guid string, imageName string, data io.Reader, mime string, caption string) error {
	if caption == ""{
		return fmt.Errorf("error: please enter a caption")
	}
