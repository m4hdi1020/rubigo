...
func (b bot) SendFile(guid string, fileName string, data io.Reader, caption string) error {
	...
	if size <= 131072 {
		...
		dataEnc, err := newSendFile(caption, guid, dcId, id, fileName, size, hashReq.Data.AccessHashRec)
		if err != nil {
			return err
		}
		body, err := newSend(b.Auth, dataEnc)
...
func (b bot) SendImage(guid string, imageName string, data io.Reader, caption string) error {
	if caption == "" {
	...
	if size <= 131072 {
		...
		dataEnc, err := newSendImage(guid, caption, dcId, id, imageName, size, imageInfo.Width, imageInfo.Height, hashReq)
		if err != nil {
			return err
		}
		body, err := newSend(b.Auth, dataEnc)
